...
public class MedianFilter implements FrameSource {
     ...
     * WINDOWED: input frames are treated as windowed data, filtered like one
     * continuos array
     */
     ...
     * edges
     * @param source The FrameSource to read from
     * @param size The size of the median filter (5 means context of 2 on each
     * side). Must be odd.
     */
     ...
     * @param size The size of the median filter (5 means context of 2 on each
     * side). Must be odd.
     * @param type The type of the input data, either WINDOWED or FRAMED
     */
    public MedianFilter(FrameSource source, int size, MedianFilter.InputType type) {
     ...
     * side). Must be odd.
     * @param type The type of the input data, either WINDOWED or FRAMED
     * @param repeatEdges Wether to repeat the edges (true) or use zeroes
     * instead (false)
     */
    public MedianFilter(FrameSource source, int size, MedianFilter.InputType type, boolean repeatEdges) {
	this.type = type;
	this.source = source;
	this.framesize = source.getFrameSize();
	this.repeat_edges = repeatEdges;
	if (size % 2 != 1) {
	    throw new IllegalArgumentException("MedianFilter: size must be odd but is " + size);
	}
	this.size = size;
	...
	public boolean read(double[] buf) throws IOException {
	    ...
	    int ringbuffer_center = (ringbuffer_start + (ringbuffer.length / 2)) % ringbuffer.length;

	    // check if we already treated the last buffer
	    if (num_cached_buffers < ringbuffer.length && current_buffer == ringbuffer_end) {
		return false;
	    }
		...
		for (int j = -half_size; j <= half_size; j++) {
			...
			case WINDOWED:
				...
				int max_offset = current_buffer - ringbuffer_start + (current_buffer - ringbuffer_start < 0 ? ringbuffer.length : 0);
				...
				if (buf_offset > max_offset) {
				    ...
				    buf_index = (buf_index + ringbuffer.length) % ringbuffer.length;
				    index = (framesize - (Math.abs(index) + framesize) % framesize);
				}
			    }
			    if (index >= framesize) {
				...
				int max_offset = ((ringbuffer_end - 1 + ringbuffer.length) % ringbuffer.length) - current_buffer + (((ringbuffer_end - 1 + ringbuffer.length) % ringbuffer.length) - current_buffer < 0 ? ringbuffer.length : 0);
				if (buf_offset > max_offset) {
			...
			case FRAMED:
			    ...
			    buf_index = (current_buffer + j + ringbuffer.length) % ringbuffer.length;
			    ...
			    max_offset = ((ringbuffer_end - 1 + ringbuffer.length) % ringbuffer.length) - current_buffer + (((ringbuffer_end - 1 + ringbuffer.length) % ringbuffer.length) - current_buffer < 0 ? ringbuffer.length : 0);
			    if (j > 0 && j > max_offset) {
				// "fell off" on the right side, no other buffer
				// there, repeat margin
				buf_index = (ringbuffer_end - 1 + ringbuffer.length) % ringbuffer.length;
				isEdge = true;
			    }
			    break;
		    }
		    // fill our temporary array to find the median value
		    medianbuf[j + half_size] = ((!isEdge || isEdge && repeat_edges) ? ringbuffer[buf_index][index] : 0.0);
		}
	    ...
	    if ( current_buffer < ringbuffer_center && (current_buffer >= ringbuffer_start || current_buffer < ringbuffer_end)) {
	...
	return "framed.filters.MedianFilter: length = " + size + ", framesize = " + framesize + ", mode: " + type.toString();
    }

    private static final String nl = System.getProperty("line.separator");
    public static final String SYNOPSIS = 
	"Apply a median filter to a series of values" + nl +
	"usage: framed.MedianFilter <mode> <context> [--zero-edges]" + nl +
	"<mode>: either \"framed\" or \"windowed\"" +
	"\tframed: input is treated as one vector per buffer which will be medianed per dimension" + nl +
	"\twindowed: input is treated as non-overlapping windowed data which will be medianed as one stream of values" + nl +
	"<context>: context to use for the calculation of the median (median-window size is 2 * <context> + 1)" + nl +
	"--zero-edges: do not repeat the values at the beginning/end of the data (default) but use zeroes instead to fill the median-window" + nl +
	nl +
	"input is read from stdin, output written to stdout, in frame format" + nl +
	"";
	...
	try {
	    type = Enum.valueOf(type.getDeclaringClass(), args[0].trim().toUpperCase());
	...
	MedianFilter filter = new MedianFilter(fis, 2 * size + 1, type, repeat_edges);
