...
//		System.err.println("strlen = " + getOrthography().length());				
	...
	public Boundary [] getB3Boundaries() {
		...
		for (Boundary b : boundaries)			
			if (b.getType().equals(Boundary.BOUNDARIES.B3))
				list.add(b);
...
//					i, getSubdivisionOrthography(i), orthography);
		...
		if (getBoundaryBeforeWord(firstWord) != null &&
				getBoundaryBeforeWord(firstWord).getType() == Boundary.BOUNDARIES.B2)
			B2before = 1;
				
		int B2after = 0;
		if (getBoundaryBeforeWord(lastWord + 1) != null &&
				getBoundaryBeforeWord(lastWord + 1).getType() == Boundary.BOUNDARIES.B2)
			B2after = 1;
		...
		if (B2before == 1)
			newBoundaries[0] = new Boundary(Boundary.BOUNDARIES.B2, 0);
		if (B2after == 1)
			newBoundaries[lastBoundary - firstBoundary + 1 + B2before] = new Boundary(Boundary.BOUNDARIES.B2,
	...
	public String toTextString(boolean includePhraseAnnotation){
		...
		if (b != null && b.getIndex() == 0 &&
				b.getType() == Boundary.BOUNDARIES.B2)
			ret += "B2 ";
