...
public class MVN implements FrameSource {
	private static Logger logger = LogManager.getLogger(MVN.class);
	...
	public static void main(String[] args) throws Exception, IOException {
		...
		ArrayList<Pair<String, String>> iolist = new ArrayList<>();
		...
		for (int i = 0; i < args.length; ++i) {
			if (args[i].equals("-h") || args[i].equals("--help")) {
				...
				iolist.add(Pair.of(args[i+1], args[i+2]));
				i += 2;
				...
				while ((line = lr.readLine()) != null) {
					...
					iolist.add(Pair.of(help[0], help[1]));
					lineCnt++;
				}
			} else {
		...
		for (Pair<String, String> p : iolist) {
			...
			if (!cumulative && parameterInputFile == null) {
				...
				work.extendStatistics(new FrameInputStream(new File(p.getLeft())));
			...
			work.setFrameSource(new FrameInputStream(new File(p.getLeft())));
			FrameOutputStream fw = new FrameOutputStream(work.getFrameSize(), new File(p.getRight()));
