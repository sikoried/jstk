...
public final class Token {
	...
	public String hierarchyAsString() {
		...
		while (agenda.size() > 0) {
			...
			// hmm stats?
			if (pair.getRight().hmm != null)
				sb.append(" id=" + pair.getRight().hmm.id);
			
			// add the children
			for (Token child : pair.getRight().moreContext)
	...
	public void dump(PrintStream ps) {
		...
		while (agenda.size() > 0) {
			...
			// print current
			ps.println(pair.getRight().toString());
			
			// hmm stats?
			if (pair.getRight().hmm != null)
				ps.println(pair.getRight().hmm);
