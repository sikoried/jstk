...
import com.github.sikoried.jstk.app.MNAP;
...
import com.github.sikoried.jstk.trans.NAP;
		...
		case MNAP: {
			...
			if (args.length == 3)
				...
				while ((line = lr.readLine()) != null) {
					...
					if (help.length != 2)
						throw new Exception("list file is broken at line " + i);
					inout.add(Pair.of(help[0], help[1]));
					i++;
				}
			}
			...
			while (inout.size() > 0) {
				...
				Mixture md = new Mixture(p.getLeft() == null ? System.in : new FileInputStream(p.getLeft()));
				...
				md.write(p.getRight() == null ? System.out : new FileOutputStream(p.getRight()));
			}
