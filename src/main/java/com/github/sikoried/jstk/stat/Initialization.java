...
import com.github.sikoried.jstk.trans.PCA;
...
public abstract class Initialization {
	public static Logger logger = LogManager.getLogger(Initialization.class);
		...
		while (c.size() < numc) {			
			...
			while (Math.abs((d1 - d2) / d1) > conv) {
				...
				for (int i = 0; i < mlea.length; ++i) {
					...
					MleDensityAccumulator.MleUpdate(c.get(i), MleDensityAccumulator.MleOptions.pDefaultOptions, Density.Flags.fAllParams, mlea[i], mlea[i].occ / sum, nd);
					c.get(i).fill(nd);
				}
