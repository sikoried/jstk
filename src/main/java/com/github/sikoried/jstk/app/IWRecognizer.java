...
public class IWRecognizer {
	private static Logger logger = LogManager.getLogger(IWRecognizer.class);
		...
		public void run() {
			try {
				...
				while ((f = distributor.next()) != null) {
					...
					for (String w : cand) {
						FrameInputStream fs = new FrameInputStream(new File(distributor.dir + System.getProperty("file.separator") + f.getLeft()));
						MetaAlignment ma = new MetaAlignment(fs, conf.tok.getSentenceTokenization(sil + " " + w + " " + sil), conf.th, true);
						hyp.add(Pair.of(ma.score, w));
						fs.close();
					}
					...
					Collections.sort(hyp, new Comparator<Pair<Double, String>>() {
						public int compare(Pair<Double, String> o1,
							...
							return (int) Math.signum(o2.getLeft() - o1.getLeft());
						}
					});
					
					// output hypotheses
					StringBuffer sb = new StringBuffer();
					sb.append(hyp.remove(0).getRight());
					for (int j = 1; j < n; ++j)
						sb.append(" " + hyp.remove(0).getRight());
	...
	public static void main(String[] args) throws Exception {
		...
		for (Pair<String, String> pp : turns)
			bw.append(pp.getLeft() + "\t" + pp.getRight() + "\n");
		bw.close();
